...
case `uname` in
    "Darwin")
        SERVICE_DEST="/Library/Application Support/FLEXnet Publisher/Service/11.11.1"
        ;;
    "Linux")
        if file ${service} | grep "ELF 64" > /dev/null
        then
            SERVICE_DEST="/usr/local/share/FNP/service64/11.11.1"
        else
            SERVICE_DEST="/usr/local/share/FNP/service/11.11.1"
        fi
        ;;
esac
...
#
fuselibs=`rpm -q -a | grep fuse-libs`
if [ "$fuselibs" ]
then
	...
	if [ $fusermountfile ] && [ -f $fusermountfile ]
	then
