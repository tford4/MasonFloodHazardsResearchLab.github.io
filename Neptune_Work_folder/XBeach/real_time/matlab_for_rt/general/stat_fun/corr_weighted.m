function R = corr_weighted(Y, w)
%CORR_WEIGHTED compute correlation coefficient using weights
%
%   corr_weighted returns a symmetric matrix R of weighted correlation
...
%   knowledge.
%
%   R = corr_weighted(Y, w) returns a positive semidefinite matrix R,
%   i.e. all its eigenvalues are non-negative (see Example 1).
%
%   corr_weighted is such that
%          corr_weighted(Y, w) == corr_weighted(a * Y + b, w)
...
%   corr_weighted(Y, w) and CORRCOEF(Y) (see Example 4).
%
%
% % ======================================================================
...
%   r1 = corr_weighted(Y, w);                                                     % Weighted Correlation matrix for Y
%
% % COMPUTE CORRELATION MATRIX FOR MODIFIED DATA
%   a = 1000 * (2 * rand - 1);                                                    % Multiplicative coefficient
%   b = 1000 * (2 * rand - 1);                                                    % Shift
%   r2 = corr_weighted(a * Y + b, w);                                             % Weighted Correlation matrix for modified Y
%
...
%   r2 = corr_weighted(repmat(a, T, 1) .* Y + repmat(b, T, 1), w);                % Weighted Correlation matrix for modified Y
%
% % FIND RELEVANT INDEXES AND PLOT A SCATTER DIAGRAM
%   I = ones(N);
%   indexes = find(tril(I, -1));                                                  % Indexes of lower triangular square matrix
...
%   r1 = corr_weighted(Y, w);                                                     % Weighted Correlation matrix
...
%   for i = 1:(T - delta + 1)
%     temp = corr_weighted(Y(i:(delta + i - 1), :), w);                           % Dynamic Weighted Correlation matrix
...
%      w = w / sum(w);
%      temp = corr_weighted(Y, w);                                                % Weighted Correlation matrix
%      r(i) = temp(2);
%   end
%   temp = corrcoef(Y);
%   rr = temp(2);
...
% $Id: corr_weighted.m 5263 2011-09-23 08:23:55Z heijer $
% $Date: 2011-09-23 10:23:55 +0200 (vr, 23 sep 2011) $
% $Author: heijer $
% $Revision: 5263 $
% $HeadURL: https://svn.oss.deltares.nl/repos/openearthtools/trunk/matlab/general/stat_fun/corr_weighted.m $
% $Keywords: $
